...
#                      Dataset exploration
# ============================================================
with h5py.File("fastmri/extracted/singlecoil_train/file1000001.h5", "r") as f:
    print("Available keys:", list(f.keys()))

# Open file
with h5py.File("fastmri/extracted/singlecoil_train/file1000001.h5", "r") as f:
    recon_rss = f['reconstruction_rss'][:]  # Array: [num_slices, H, W]
    recon_esc = f['reconstruction_esc'][:]  # Array: [num_slices, H, W]

# Print image and show a slice
print("RSS structure:", recon_rss.shape)
print("ESC structure:", recon_esc.shape)

# Show the first slice
id = 1
plt.subplot(1,2,1)
plt.imshow(recon_rss[id], cmap='gray')
plt.title(f" Reconstructed MRI(RSS) - slice {id}")
plt.axis("off")

print(recon_rss[15].shape
    )

plt.subplot(1,2,2)
plt.imshow(recon_esc[id], cmap='gray')
plt.title(f"        Reconstructed MRI(ESC) - slice {id}")
plt.axis("off")
plt.show()
...
def precompute_norm_stats(files: List[str], recon="rss", out_json="norm_stats.json",
    ...
    """
    Computes vmin/vmax per-volume (sampling some downsampled slices) e saves on JSON.
...
# -------------------------------------------------------------------------------
def sample_svnr(imgs, sigma, gamma_schedule, test=False):
    """
    ...
    # 1) gamma_T
    sigma_p = torch.sqrt(sigma**2 * imgs01).clamp(min=1e-3)
...
# ----------------------------------------------------------
#       Computational training and validation time count
# ----------------------------------------------------------
...
for epoch in range(num_epochs):
    ...
    for imgs, _ in pbar:
        ...
        noisy, xt, tmap, eps_tilde,sigma_p = sample_svnr(
            imgs, sigma_train, gamma_schedule
        )
    ...
    if epoch_train_loss < best_train_loss:
        ...
        torch.save(svnr_model.state_dict(), "MRI_one_sigma_PRO_5_lowest.pth")
    ...
    if (epoch + 1) % 10 == 0:
        ...
        with torch.no_grad():
            for imgs_v, _ in val_loader:
                ...
                noisy_v, _, _, _,_ = sample_svnr(imgs_v, sigma_val, gamma_schedule)
                if device.type == "cuda":
        ...
        if save_flag:
            torch.save(svnr_model.state_dict(), "MRI_one_sigma_PRO_5_best.pth")
            epoch_best = epoch
            print(
...
# Optional plot save
plt.savefig("loss_MRI_one_sigma_PRO_5.png")
print("Saved plot to loss_MRI_one_sigma_PRO_5.png")
...
# ----------------------------------------------
svnr_model.load_state_dict(torch.load("MRI_one_sigma_PRO_5_best.pth",  map_location=device),strict=False) # MRI_one_sigma_PRO_lowest.pth for lowest loss
svnr_model.eval()                                                                                                                                                  
...
def tens_to_np01(x):
    """Tensor [1,1,H,W] or [1,H,W] -> numpy [H,W] in [0,1]"""
    if x.dim() == 4:
        x = x[0, 0]
    elif x.dim() == 3:
        x = x[0]
...
for test_id, sigma in enumerate(SIGMA_TESTS, 1):
    ...
    with torch.no_grad():
        ...
        for start in range(0, N, BATCH):
            ...
            imgs = test_imgs[start:end].to(device)  # [B,1,H,W] in [-1,1]

            # 1) Forward SVNR: sample (noisy, xt, tmap)
            noisy, xt, tmap, _ , _= sample_svnr(imgs, sigma_val, gamma_schedule)
            ...
            gt01  = to01(imgs)
            den01 = to01(denoised)
            gt_np  = gt01.detach().cpu().numpy()   # [B,1,H,W]
            den_np = den01.detach().cpu().numpy()
            ...
            for vis_idx in VISUAL_IDXS:
                if start <= vis_idx < end:
                    ...
                    noisy_np = to01(noisy[local:local+1]).detach().cpu().numpy()[0,0]
    ...
    for k in range(n_show):
        ...
        plt.tight_layout()
        plt.savefig(f"triptych_test{test_id}_slice{VISUAL_IDXS[k]}.png", dpi=300) # Optional figure save
        plt.show()
    ...
    if psnr_list:
        ...
        plt.tight_layout()
        plt.legend()
        plt.savefig(f"hist_psnr_test_one_sigma_PRO_5_{test_id}.png", dpi=300) # Optional figure save
        plt.show()
    ...
    if ssim_list:
        ...
        plt.tight_layout()
        plt.legend()
        plt.savefig(f"hist_ssim_test_one_sigma_PRO_5_{test_id}.png", dpi=300) # Optional figure save
        plt.show()
...
# Forward SVNR
xt, tmap = sample_svnr(y, sigma_val, gamma_schedule, test=True)
...
# Optional plot save
plt.savefig("20_sets_MRI_one_sigma_PRO_5_rss.png")
print("Saved plot to 20_sets_MRI_one_sigma_PRO_5_rss.png")
...
out_path = "RSS_direct_denoising_4examples.png"
fig.savefig(out_path, dpi=300, bbox_inches="tight", pad_inches=0.05) # Optional figure save
